...
# === CONFIGURATION ===
# NOTE: API keys are intentionally obfuscated for security purposes.
...
def show_gif_animation(gif_path):
    def start_animation():
        ...
        window = Tk()
        window.title("Meer AI") # Updated title
...
def persistent_interruption_listener():
    with sr.Microphone() as source:
        ...
        while True:
            try:
                # 📢 CHANGED: Listening for 'Meer stop' command
                print("🎧 Listening for 'Meer stop' command...")
                audio = recognizer.listen(source, timeout=2, phrase_time_limit=3)
                said = recognizer.recognize_google(audio)
                print("🔈 Interruption Listener Heard:", said)
                # 📢 CHANGED: Interruption trigger is 'meer stop'
                if "meer stop" in said.lower():
                    interrupt_event.set()
                    engine.stop()
...
def chat_with_deepseek(prompt):
    headers = {
        ...
        "X-Title": "Meer Assistant" # Updated Title
    }
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are Meer, a helpful voice assistant. Respond clearly and briefly using the provided context." # Updated Role
            },
            {
                "role": "user",
...
def main():
    ...
    # 📢 CHANGED: Introduction and wake word
    speak("Hello, I am Meer. Say 'Meer' to activate me.")
    while True:
        # 📢 CHANGED: Waiting for wake word 'Meer'
        print("🕒 Waiting for wake word 'Meer'...")
        wake_input = listen()
        # 📢 CHANGED: Check for wake word 'meer'
        if wake_input and "meer" in wake_input.lower():
            ...
            if any(word in command_lower for word in ["exit", "quit", "stop", "bye"]):
                ...
                # NOTE: Remember to change "YourUsername" to your actual Windows username for this to work
